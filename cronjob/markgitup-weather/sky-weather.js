...
(function (root) {
    ...
    function weatherFrom(data, now) {
        ...
        if (!description || !Number.isFinite(current.time) || !Number.isFinite(current.cloud_cover)
            ...
            || current.temperature_2m < -100 || current.temperature_2m > 70
        ...
        if (data.current_units?.temperature_2m !== '°C') throw new Error('Unexpected weather units');
        ...
        return {...description,code:current.weather_code,cloud:current.cloud_cover/100,
            temperature:current.temperature_2m,precipitation:current.precipitation,
    ...
    async function fetchSnapshot({fetchImpl=root.fetch.bind(root),now=new Date(),timeoutMs=4000}={}) {
        const location = locationFrom(await fetchJSON('https://get.geojs.io/v1/ip/geo.json',fetchImpl,timeoutMs));
        ...
        const weather = weatherFrom(await fetchJSON(`https://api.open-meteo.com/v1/forecast?${query}`,fetchImpl,timeoutMs),now);
